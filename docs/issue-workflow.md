...
逐个启动修复（用户确认顺序）
  ...
  commit 包含 refs #N（⚠️ 不用 closes，Issue 保持开启）
...
更新 BugIssueTestCase.md
  追加验收 TC 块，等待真机回归
        ...
        ▼
【新】回归验收确认（用户操作）
  测试人员安装 GitHub Release 页面发布的 APK
  执行 BugIssueTestCase.md 中对应 TC 用例
  验证通过 → 在 GitHub Issue 上手动关闭 Issue
        │
        ▼
【自动】regression.yml 触发
  监听 issue closed 事件（label = bug）
  → 自动在 Issue 追加关闭评论，记录版本号
  → 通知 Claude Code 更新测试报告
        │
        ▼
告诉 Claude Code："#N 回归通过"
  → Claude Code 在 BugIssueTestCase.md 对应 TC 打勾并注明版本号
  → 更新 tc-*.md 原始用例为 [x] 通过
  → 提交 commit：docs: #N 回归验收通过，更新 tc-*.md
```
...
**Commit 格式**（⚠️ 使用 `refs` 而非 `closes`，避免 Issue 被自动关闭）：
```
fix: 修复统计页 combine 溢出导致崩溃 refs #42
...
> **为什么不用 `closes #N`？**
> GitHub 检测到 `closes #N` 后，commit 合并到 master 时会立即自动关闭 Issue。
> 此时回归验收还未进行，Issue 不应提前关闭。
> 改用 `refs #N` 只建立关联引用，Issue 保持开启，等待真机回归确认后由用户手动关闭。
...
GitHub Actions 自动构建并发布 release APK，并在 Issue 上追加评论说明已发版（Issue 保持开启）。
...
4. **在 GitHub Issue 页面点击「Close issue」手动关闭 Issue**（这一步触发自动化流程）
...
用户关闭 Issue 后，`regression.yml` 自动执行：

1. 检测 Issue 是否带有 `bug` label
2. 在 Issue 自动追加评论：`✅ Issue 已关闭，请告知 Claude Code 更新测试报告`

**Claude Code 收到"回归通过"后自动执行**：

1. 在 GitHub Issue 追加评论：`✅ v{版本号} 回归验收通过`
...
GitHub Issue（追踪 & 修复过程记录）
      │
      │ refs #N（commit 合并，Issue 保持开启）+ 打 tag 触发发版
      ▼
...
真机安装 release APK，回归验收
      │
      │ 验证通过 → 用户在 Issue 页面手动关闭
      ▼
regression.yml 自动触发（issue closed + label=bug）
      │
      │ 追加关闭评论，提醒告知 Claude Code
      ▼
告知 Claude Code "#N 回归通过"
      │
      │ 更新 BugIssueTestCase.md + tc-*.md
      ▼
提交 docs commit，Issue 完整归档
```
