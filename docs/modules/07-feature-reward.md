...
- 家长创建奖品时设置：**所需总积分**（如 100）和**拼图总块数**（如 20）
- 系统自动计算每块门槛：`pointsPerPiece = ceil(requiredPoints / puzzlePieces)`，最小为 1
- 兑换时，用户选择消耗的蘑菇类型和数量
- 贡献积分 = 蘑菇数量 × 该等级的兑换积分值（小蘑菇=1，中=5，大=25，金=100，传说=500）
- 可解锁块数 = `min(贡献积分 / pointsPerPiece, 剩余未解锁块数)`（整除，积分不足一块则失败）
- UI 实时预览：选择蘑菇类型和数量时，显示「贡献积分 X 分 → 可解锁 Y 块」
